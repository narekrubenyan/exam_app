        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">{{ __('dashboard.categories.categoriesTable') }}</h3>
                    <div class="card-tools">
                        <div>
                            <a href="{{ route('categories.create') }}" class="btn btn-block btn-primary">{{ __('dashboard.categories.addCategory') }}</a>
                        </div>
                    </div>
                </div>

                @if (!count($categories))
                    <div class="card-body"><h1><b>0</b> {{ __('dashboard.categories.category') }}</h1></div>
                @else
                            ...
                            <tbody>
                                @foreach ($categories as $category)
                                    <tr>
                                        ...
                                        <td>{{ $category->name }}</td>
                                        <td>
                                            <span><a href="{{ route('categories.edit', $category->id) }}" class="btn btn-warning">{{ __('dashboard.edit') }}</a></span>
                                            <span>
